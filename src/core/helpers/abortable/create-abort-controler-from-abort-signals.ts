...
import { addEventListener, RemoveEventListener } from '../event-listener/add-event-listener';
...
export function CreateAbortControllerFromAbortSignals(
  ...
  const unsubscribeOwnSignal: RemoveEventListener = addEventListener(abortController.signal, 'abort', clear);

  const unsubscribeSignalList: RemoveEventListener[] = signals.map((signal: AbortSignal) => {
    return addEventListener(signal, 'abort', () => {
      clear();
      abortController.abort();
    });
  });
